...
//check to see that the Quote Type variable is set, if not send them back to the form
if ((!isset($_POST['quoteType'])) && (!isset($_SESSION['loadsaved']))) {
    header("Location: estimate.php");
    exit;
}

//check to see which kind of proccessing we need: estimate, ballpark or retrieval
$qt = $_POST['quoteType'];

//check if we're loading a saved session or not
if (isset($_SESSION['loadsaved']) && ($_SESSION['loadsaved'] == 'true')) {
    // load the xml file and parse the data into the objects
    if (isset($_SESSION['reloadedXML'])) {
        ...
        if ($xmlDoc->load($filePath)) {
            ...
            $output = parse_language_table($languageTbl, $dataDump->getElementsByTagName('languages')->item(0));
            $_SESSION['languageTbl'] = $languageTbl; //add the language table to the session
            //build the estimate object
            $estimate = new estimate;
            parse_estimate($estimate, $dataDump->getElementsByTagName('estimate')->item(0));
            $_SESSION['estimate'] = $estimate; //put the estimate object into the session
            //build the Rep/contact object
            $lingoContact = new contact;
            parse_contact($lingoContact, $dataDump->getElementsByTagName('lingocontact')->item(0));


            //build the PM object
            $lingoPM = new contact;
            parse_contact($lingoPM, $dataDump->getElementsByTagName('lingopm')->item(0));
            ...
            $err_msg = "<h3>Error: Could not open XML file. Please try again.</h3>";
            unset($xmlDoc);
            $output = -1;
        }
    } else {
        $err_msg = "<h3>Error: No file path found</h3>";
        $output = -1;
    }
} elseif (($qt == 'qtBallpark') || ($qt == 'qtEstimate')) {
    //first thing, populate the tempData class, in case we need to "go back"
    $tempObj = new tempData;
    populate_temp($tempObj);
    $_SESSION['temp'] = $tempObj;
    ...
    //we have an unknown type, so go back to the form
    header("Location: estimate.php");
    exit;
}
?>
...
<html xmlns="http//www.w3.org/1999/xhtml">
    <head>
        ...
        <script src="./libs/costper.js"></script>
...
if ($output == -1) {
    ...
    print ("<form name=\"resultForm\" method=\"post\" action=\"s.php\"><br />\n");
    ...
    /* echo "<div style=\"width:100%; text-align:center;\"><input type=\"button\" value=\"Unlock cost units\" id=\"unlockCosts\"";
      if (isset($_SESSION['unlockCosts']) && ($_SESSION['unlockCosts'] == true))
      echo " disabled ";
      else
      echo " onClick=\"unlockCosts();\" ";
      echo "></div>"; */
    ...
    for ($lcv = 0; $lcv < $output; $lcv++) {
        ...
        if ($languageTbl[$lcv]->get_error() == TRUE) {
            ...
            for ($columns = 0; $columns < count($languageTbl[$lcv]->get_Tasks()); $columns++) {
                ...
                if ($columns == PM)
                    $languageTbl[$lcv]->get_task($columns)->print_table_row($lcv, $columns, $languageTbl[$lcv], true);
                else
                    $languageTbl[$lcv]->get_task($columns)->print_table_row($lcv, $columns, true);
            }
