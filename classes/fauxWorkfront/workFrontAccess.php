...
CHARACTER SET latin1 COLLATE latin1_bin NULL DEFAULT NULL";
  ...
  dtBucket varchar(100) DEFAULT '')";
    const DROP_ADDITIONAL_TASKS = "DROP TABLE IF EXISTS additionaltasks";
    const CREATE_ADDITIONAL_TASKS_TABLE = "CREATE TABLE additionaltasks (
  task_order int PRIMARY KEY,
  task_name varchar(50) unique key not null,
  task_class varchar(50) not null,
  task_type varchar(50) not null,
  linguist_type varchar(50) DEFAULT '')";
    const ADDITIONAL_TASKS_INSERT_STATEMENT = "insert into additionaltasks (task_order, task_name, task_class, task_type, linguist_type) 
    values
    (1,'Voice-Over Recording', 'linguistic', 'VO', 'Hourly Miscellaneous Task'),
    (2,'Synching', 'billable','Localization Engineer', ''),
    (3,'Transcription', 'linguistic', 'TCP', 'Trados TR/TR+CE Task'),
    (4,'Back-Translation', 'linguistic', 'BT', 'Trados TR/TR+CE Task'),
    (5,'Miscellaneous', 'billable','Localization Engineer', '')";
    ...
    public function geAdditionalTasks() {
        $this->myDBConnection = new PricingMySql();

        echo "Dropping additionaltasks table if exists...\n";
        $this->queryExecute($this->myDBConnection, self::DROP_ADDITIONAL_TASKS);
        echo "Drop of additionaltasks table complete.\n";
        echo "Creating additionaltasks table...\n";
        $this->queryExecute($this->myDBConnection, self::CREATE_ADDITIONAL_TASKS_TABLE);
        echo "Creation of additionaltasks table complete.\n";
        echo "Beginning insert additionaltasks data process...\n";
        $this->queryExecute($this->myDBConnection, self::ADDITIONAL_TASKS_INSERT_STATEMENT);
        echo "Insert additionaltasks data and table generation complete.\n";
    }
