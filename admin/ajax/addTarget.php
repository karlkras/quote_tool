...
else
{
	...
	$query = "INSERT INTO targetlang (Language) VALUES ('". $languageName . "')";
	$result = $myDBConn->query($query);
	
	if (!$result)
	{	
		$error = "TRUE";
		$errortext = "Could not insert target language into database.\n".$myDBConn->error;	
	}
	else
	{
		$error = "FALSE";
		...
		$query = "SELECT ID FROM targetlang WHERE Language='". $languageName ."'";
		$result = $myDBConn->query($query);
